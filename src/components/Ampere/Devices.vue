      ...
      <section class="content-container">
        ...
        <section class="table-container">
          <el-table border :data="dataCollection" max-height="500"
                    ...
                    ref="multipleTable">
            <el-table-column :label="$t('ElectricCurrent.table.create')" prop="time" align="center" width="200"
                             show-overflow-tooltip sortable></el-table-column>
            <el-table-column :label="$t('ElectricCurrent.table.aPhase')" prop="resultA" align="center" width="100"
                             show-overflow-tooltip></el-table-column>
            <el-table-column :label="$t('ElectricCurrent.table.bPhase')" prop="resultB" align="center" width="100"
                             show-overflow-tooltip></el-table-column>
            <el-table-column :label="$t('ElectricCurrent.table.cPhase')" prop="resultC" align="center" width="100"
                             show-overflow-tooltip></el-table-column>
          </el-table>
  ...
  export default {
    ...
    methods: {
      handleWebSocketOnMessage (event) {
        ...
        console.log(`电流Websocket中uid为：${ret.uid},用户选择的uid为：${_that.deviceUid}`)
        if (ret.uid === _that.deviceUid) {
          _that.title.push(formatDate(ret.datetime, "hh:mm:ss"))
          _that.APhase.push(ret.a_ampereU)
          _that.BPhase.push(ret.a_ampereV)
          _that.CPhase.push(ret.a_ampereW)

          _that.dataCollection.unshift({
            time: formatDate(ret.datetime, 'yyyy-MM-dd hh:mm:ss'),
            resultA: ret.a_ampereU,
            resultB: ret.a_ampereV,
            resultC: ret.a_ampereW
          })
          ...
          console.log(`电流UID不相等，Websocket中uid为：${ret.uid},用户选择的uid为：${_that.deviceUid}`)
        }
      },
      handlePickerChange () {
      ...
      setChartOptions () {
        let _that = this
        console.log(_that.APhase,'hdn111');
        _that.myChart.setOption({
          title: {
            text: _that.$t('ElectricCurrent.text.title'),
            left: '50%',
            textStyle: {
              fontSize: 14,
              color: '#4A5B79',
          ...
          yAxis: {
            name: _that.$t('ElectricCurrent.text.yTitle'),
            nameLocation: 'middle',
            nameGap: 35
          },
          series: [
            {
              type: 'line',
              name: _that.$t('ElectricCurrent.table.aPhase'),
              symbol: 'rect',
              symbolSize: 6,
              smooth: true,
              data: _that.APhase,
            ...
            {
              type: 'line',
              name: _that.$t('ElectricCurrent.table.bPhase'),
              symbol: 'triangle',
              symbolSize: 6,
              smooth: true,
              data: _that.BPhase,
            ...
            {
              type: 'line',
              name: _that.$t('ElectricCurrent.table.cPhase'),
              symbol: 'circle',
              symbolSize: 6,
              smooth: true,
              data: _that.CPhase,
