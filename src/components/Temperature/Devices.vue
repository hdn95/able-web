<!--电机数据温度页面-->
<template>
  <section class="temperatures-wrapper-container">
      ...
      <section class="content-container">
        ...
        <section class="table-container">
          <el-table border :data="dataCollection" max-height="500"
                    ...
                    ref="multipleTable" style="width: 32rem">
            <el-table-column :label="$t('Temperature.table.create')" prop="time" align="center" width="200"
                             show-overflow-tooltip sortable fixed></el-table-column>
            <el-table-column :label="$t('Temperature.table.terminalBox')" prop="result0" align="center"
                             show-overflow-tooltip width="80"></el-table-column>
            <el-table-column :label="$t('Temperature.table.statorWindingU')" prop="result1" align="center"
                             show-overflow-tooltip width="80"></el-table-column>
            <el-table-column :label="$t('Temperature.table.statorWindingV')" prop="result2" align="center"
                             show-overflow-tooltip width="80"></el-table-column>
            <el-table-column :label="$t('Temperature.table.statorWindingW')" prop="result3" align="center"
                             show-overflow-tooltip width="80"></el-table-column>
            <el-table-column :label="$t('Temperature.table.frontBearing')" prop="result4" align="center"
                             show-overflow-tooltip width="80"></el-table-column>
            <el-table-column :label="$t('Temperature.table.realBearing')" prop="result5" align="center"
                             show-overflow-tooltip width="80"></el-table-column>
          </el-table>
  ...
  export default {
    ...
    methods: {
      handleWebSocketOnMessage (event) {
        ...
        console.log(`温度Websocket中uid为：${ret.uid},用户选择的uid为：${_that.deviceUid}`)
        if (ret.uid === _that.deviceUid) {
          _that.title.push(formatDate(ret.datetime, "hh:mm:ss"))
          _that.result0.push(ret.t_temp0)
          _that.result1.push(ret.t_temp1)
          _that.result2.push(ret.t_temp2)
          _that.result3.push(ret.t_temp3)
          _that.result4.push(ret.t_temp4)
          _that.result5.push(ret.t_temp5)
          ...
          console.log(`温度UID不相等，Websocket中uid为：${ret.uid},用户选择的uid为：${_that.deviceUid}`)
        }
      },
      handlePickerChange () {
        ...
        _that.$fetch(`${_that.$route.path}`, params)
          .then((ret) => {
            ...
            retData.items.forEach((item) => {
              ...
              result0.unshift(item.result0)
              result1.unshift(item.result1)
              result2.unshift(item.result2)
              result3.unshift(item.result3)
              result4.unshift(item.result4)
              result5.unshift(item.result5)

            })

            _that.title = title
            _that.result0 = result0
            _that.result1 = result1
            _that.result2 = result2
            _that.result3 = result3
            _that.result4 = result4
            _that.result5 = result5
      ...
      handleChartOptions () {
        ...
        if (_that.myChart === undefined) {
          _that.myChart = _that.$echarts.init(document.getElementById('myChart'))
        }
      ...
      setChartOptions () {
        ...
        _that.myChart.setOption({
          title: {
            text: _that.$t('Temperature.text.title'),
            left: '50%',
            textStyle: {
              fontSize: 14,
              color: '#4A5B79',
          ...
          yAxis: {
            name: _that.$t('Temperature.text.yTitle'),
            nameLocation: 'middle',
            nameGap: 35
          },
          series: [
            {
              type: 'line',
              name: _that.$t('Temperature.table.terminalBox'),
              symbol: 'rect',
              symbolSize: 6,
              smooth: true,
              data: _that.result0,
            ...
            {
              type: 'line',
              name: _that.$t('Temperature.table.statorWindingU'),
              symbol: 'triangle',
              symbolSize: 6,
              smooth: true,
              data: _that.result1,
            ...
            {
              type: 'line',
              name: _that.$t('Temperature.table.statorWindingV'),
              symbol: 'circle',
              symbolSize: 6,
              smooth: true,
              data: _that.result2,
            ...
            {
              type: 'line',
              name: _that.$t('Temperature.table.statorWindingW'),
              symbol: 'emptyCircle',
              symbolSize: 6,
              smooth: true,
              data: _that.result3,
            ...
            {
              type: 'line',
              name: _that.$t('Temperature.table.frontBearing'),
              symbol: 'arrow',
              symbolSize: 6,
              smooth: true,
              data: _that.result4,
            ...
            {
              type: 'line',
              name: _that.$t('Temperature.table.realBearing'),
              symbol: 'diamond',
              symbolSize: 6,
              smooth: true,
              data: _that.result5,
...
<style type="text/css" scoped>
  ...
  .temperatures-wrapper-container {
    display: flex;
    display: -webkit-flex;
    display: -moz-flex;
  }
